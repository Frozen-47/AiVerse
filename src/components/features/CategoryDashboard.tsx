...
export const CategoryDashboard: React.FC<CategoryDashboardProps> = ({
  ...
  return (
    <div
      id="categories"
      className={`p-6 sm:p-8 rounded-[28px] border backdrop-blur-md transition-all scroll-mt-24 ${t.surface} shadow-lg`}
    >
      {/* Section heading */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-3">
            <span className={`text-[10px] font-extrabold tracking-widest uppercase px-2.5 py-1 rounded-full border ${t.pillInactive}`}>
              Interactive Hub
            </span>
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/80 animate-pulse" />
          </div>
          <h2 className={`text-2xl sm:text-3xl font-black tracking-tight mb-2 ${t.textPrimary}`}>
            Discover the AI Universe
          </h2>
        ...
        <div className="shrink-0 flex items-center">
          <button
            ...
            className={`w-full md:w-auto relative group overflow-hidden px-6 py-4 rounded-2xl border text-center font-bold tracking-wide transition-all duration-300 flex items-center justify-center gap-3 ${t.btnSecondary}`}
          >
            <div className="relative z-10 flex items-center gap-2">
              <BookOpen size={16} className="transition-colors" />
              <span className="text-[14px]">Browse All {entries.length} AI Tools</span>
              <span className={`text-[10px] px-2 py-0.5 rounded-md border ${t.pillSmall}`}>
                EXPLORE
              </span>
            </div>
          </button>
        ...
        {categories.map((cat, i) => {
          ...
          return (
            <button
              ...
              className={`group relative flex flex-col items-start p-5 rounded-2xl transition-all duration-300 cursor-pointer ${t.card}`}
            >
              <div className="absolute top-4 right-4">
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md border ${t.pillSmall}`}>
                  {cat.count} items
                </span>
              </div>
              <div className={`p-2.5 rounded-xl ${t.iconBgSolid} mb-4 transition-colors`}>
                <Icon className="w-5 h-5" />
              </div>
              <h3 className={`font-bold text-[14px] mb-1 transition-colors ${t.textPrimary}`}>
                {cat.title}
              </h3>
